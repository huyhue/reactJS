...
class Contact extends Component {
  constructor(props) {
    ...
    this.state = {
      isRedirect: false,
      fNgay: "thu7" //tạo giá trị mặc định
    }
  }

  isChange = (event) => {
    const ten = event.target.name;
    const giatri = event.target.value;  //giá trị khi bạn nhập vào form
    this.setState({
      [ten]: giatri  //Handling Multiple Inputs
    });
  }
  
  submitForm = (event) => {
    event.preventDefault();  //Ngăn chặn các thao tác mặc định
    this.setState({
      isRedirect: true
    });
  }

  isFileChange = (event) => {  //Với kiểu file nó có chút khác
  ...
  getGiaTri = () => {
    ...
    noiDung += ". Ảnh nhận được là: " + this.state.fAnh;
    return noiDung;
  }
